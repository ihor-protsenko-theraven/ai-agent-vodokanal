...
export class TranscriptPanelComponent {
  ...
  public render(): void {
    ...
    this.container.innerHTML = `
      <div class="h-full flex flex-col bg-slate-900 border-r border-slate-800/80 overflow-hidden relative">
        
        <!-- Processing Loading Overlay -->
        ${isProcessing ? `
          <div class="absolute inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center space-y-4">
        ...
        : 'bg-rose-600 hover:bg-rose-500 text-white shadow-rose-600/20'
          ...
          <div class="grid grid-cols-2 gap-2 pt-1 border-t border-slate-800/80">
            <button id="btn-record-mic" class="py-2 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 ${this.isRecording
        ...
        : 'bg-gradient-to-r from-sky-600 to-sky-500 hover:from-sky-500 hover:to-sky-400 text-white shadow-md shadow-sky-600/20'
              ...
              ${this.isRecording ? '⏹️ Зупинити запис' : '🎙️ Запис з мікрофона'}
            </button>

            <label for="input-audio-file" class="py-2 px-3 rounded-xl text-xs font-bold bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 transition-all flex items-center justify-center gap-2 cursor-pointer select-none">
              📁 Файл (.mp3/.wav)
          ...
          <div class="bg-slate-900/90 rounded-xl p-3 border border-slate-800 flex items-center gap-3">
            <button id="btn-play-audio" class="w-9 h-9 rounded-lg bg-sky-600 hover:bg-sky-500 text-white flex items-center justify-center transition-all shadow-md shadow-sky-600/30 flex-shrink-0">
        ...
        : '<path d="M8 5v14l11-7z"/>'}
            ...
            <div class="flex-1 flex items-center gap-1 h-8 px-2 overflow-hidden">
              ${Array.from({ length: 32 }).map((_, i) => `
                <div class="w-1 bg-sky-400/80 rounded-full transition-all duration-300 ${this.isPlayingAudio || this.isRecording ? 'animate-wave-bar' : 'h-2 opacity-40'}" style="animation-delay: ${(i * 0.05).toFixed(2)}s; height: ${this.isPlayingAudio || this.isRecording ? Math.floor(Math.random() * 20 + 8) + 'px' : '6px'}"></div>
              `).join('')}
            </div>
          ...
          <div class="flex items-center justify-between bg-slate-900 p-2 rounded-xl border border-slate-800">
            <span class="text-xs font-medium text-slate-300">Режим роботи AI-агента:</span>
            <div class="flex bg-slate-950 p-1 rounded-lg border border-slate-800">
              <button data-mode="TEXT_HINTS" class="btn-mode-toggle text-xs font-semibold px-3 py-1 rounded-md transition-all ${mode === 'TEXT_HINTS'
        ? 'bg-sky-600 text-white shadow-sm'
        : 'text-slate-400 hover:text-slate-200'
      }">
                💬 Текстові підказки
              </button>
              <button data-mode="VOICE_DIALOG" class="btn-mode-toggle text-xs font-semibold px-3 py-1 rounded-md transition-all ${mode === 'VOICE_DIALOG'
        ? 'bg-sky-600 text-white shadow-sm'
        : 'text-slate-400 hover:text-slate-200'
      }">
                🗣️ Голосовий діалог
        ...
        <div class="flex-1 p-4 overflow-y-auto space-y-3.5 bg-slate-900/50">
        ...
        return `
              <div class="flex gap-3 text-xs leading-relaxed ${isAI ? 'justify-start' : 'justify-start'}">
                <div class="w-7 h-7 rounded-lg ${isAI ? 'bg-sky-500/20 text-sky-400 border border-sky-500/30' : 'bg-slate-800 text-slate-300 border border-slate-700'} flex items-center justify-center flex-shrink-0 font-bold text-[10px]">
                  ${isAI ? 'AI' : 'УБ'}
                </div>
        ...
        <div class="p-4 bg-slate-950 border-t border-slate-800 flex flex-col gap-2">
          <div class="flex items-center justify-between">
            <h3 class="text-xs font-bold text-sky-400 flex items-center gap-1.5">
              ...
              Рекомендовані питання від AI (suggestedQuestions):
            </h3>
          ...
          <div class="flex flex-col gap-1.5">
            ${suggestedQuestions.length > 0 ? suggestedQuestions.map(q => `
              <button data-question="${encodeURIComponent(q)}" class="btn-suggested-question text-left text-xs bg-sky-950/40 hover:bg-sky-900/60 border border-sky-800/60 hover:border-sky-500/80 text-sky-200 p-2.5 rounded-lg transition-all flex items-center justify-between group">
